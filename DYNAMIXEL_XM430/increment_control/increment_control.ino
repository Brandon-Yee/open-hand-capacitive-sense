...
void loop() {
  ...
  while (Serial.available())
  {
    ...
    dxl_wb.itemRead(DXL_ID, "Present_Position", &present_pos);
    bool goal_read_result = dxl_wb.itemRead(DXL_ID, "Goal_Position", &goal_pos);
    if (goal_read_result)
    {
      new_goal_pos = ('j' == input) ? goal_pos - INCR : goal_pos;
      new_goal_pos = ('k' == input) ? goal_pos + INCR : new_goal_pos;
    }
